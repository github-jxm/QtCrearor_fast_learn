...
//     QFinalState * sEnd = new QFinalState(&machine);

    // 为按钮部件的geometry属性分配一个值，当进入该状态时会设置该值
    s1->assignProperty(&button, "geometry", QRect(100, 100, 100, 50));
    s2->assignProperty(&button, "geometry", QRect(300, 100, 200, 50));
    s3->assignProperty(&button, "geometry", QRect(200, 200, 300, 500));
    ...
    // QSignalTransition *transition3 = s3->addTransition(&button, SIGNAL(clicked()), sEnd);
