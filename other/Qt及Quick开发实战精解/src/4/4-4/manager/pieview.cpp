...
void PieView::paintEvent(QPaintEvent *event)
{
        ...
        if (validItems > 0) {
                ...
                painter.drawEllipse(0, 0, pieSize, pieSize);  /* 绘制 椭圆   画个圆圈*/
                double startAngle = 0.0;  // 起始角度
                int row;

                for (row = 0; row < model()->rowCount(rootIndex()); ++row) {

                        QModelIndex index = model()->index(row, 1, rootIndex());  /*数据index*/
                        double value = model()->data(index).toDouble();

                        if (value > 0.0) {
                                double angle = 360 * value/totalValue;   // 求 角度大小

                                QModelIndex colorIndex = model()->index(row, 0, rootIndex());  /* 颜色index*/
                                QColor color = QColor(model()->data(colorIndex,    /* 颜色值 */
                                ...
                                painter.drawPie( 0,  0,  pieSize,  pieSize,  int(startAngle * 16),  /*画扇形*/
                                                             int(angle * 16)  );
